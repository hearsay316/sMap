...
function analysisSymbolTree(symbolLibManager) {
  ...
  for (var i = 0; i < symbolLibManager.getSymbolLibNumber(); i++) {
    ...
    if (rootSymbolInfo.symbolNodeType === "SYMBOL_GROUP") {
      ...
      idIndex = innerAnalysisSymbolTree(
...
function addBasicCellTreeNodes(treeData) {
  ...
  var symbolCode = [
    ...
    /*1024, 321,1023,*/ 1025,
    1013,
    1014 /*1016, 1017, 1026,*/ /*1001 1003, 1004*/
  ];
  var symbolName = [
    "折线",
    "平行四边形",
...
function innerAnalysisSymbolTree(
  ...
  for (var i = 0; i < childSymbolInfos.length; i++) {
    if (childSymbolInfos[i].symbolNodeType === "SYMBOL_GROUP") {
      ...
      treeNode.fullName =
        parentNode.fullName + childSymbolInfos[i].symbolName + "/";
      treeData.push(treeNode);

      treeNodeId = innerAnalysisSymbolTree(
        childSymbolInfos[i].childNodes,
        treeData,
        treeNode,
        rootSymbolIconUrl
      );
      ...
      drawNode.icon =
        rootSymbolIconUrl +
        parentNode.fullName +
        childSymbolInfos[i].symbolCode +
        ".png";
      drawNode.symbolCode = childSymbolInfos[i].symbolCode;
      drawNode.libID = childSymbolInfos[i].libID;
      drawNode.symbolName =
        childSymbolInfos[i].symbolName + "_" + childSymbolInfos[i].symbolCode;
